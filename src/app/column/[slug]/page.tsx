...
export function generateStaticParams() {
  return getAllSlugs('column').map((slug) => ({ slug }))
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const a = getArticle('column', slug)
  if (!a) return {}
  return {
    title: `${a.title}｜コラム`,
    description: a.description || a.title,
    alternates: { canonical: `${SITE_URL}/column/${a.slug}` },
    openGraph: {
      type: 'article',
...
export default async function ColumnPage({ params }: Props) {
  ...
  // 関連記事（同タグ最大3本、自分以外）
  const related = getAllArticles('column')
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: a.title, item: `${SITE_URL}/column/${a.slug}` },
    ],
  }
