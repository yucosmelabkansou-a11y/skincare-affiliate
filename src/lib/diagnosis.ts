...
export const resultTypes: Record<SkinType, ResultType> = {
  dry: {
    ...
    bgColor: '#EAF0F2',
    description:
      '肌の水分量が不足しがちな状態。洗顔後のつっぱりや粉ふき・カサつきが起きやすく、乾燥が続くと小ジワが目立ちやすい傾向があります。「水分を与える」と「うるおいを逃さないフタをする」の両輪が鍵。',
    features: [
      '洗顔後に肌がつっぱりやすい',
      '頬や口周りがカサつく',
      '乾燥による小ジワが目立ちやすい',
    ...
    cares: [
      { icon: '💧', title: 'セラミドでうるおい補給', text: 'セラミドやヒアルロン酸配合の化粧水で角層にうるおいを与える' },
      { icon: '🧴', title: 'クリームでうるおいを閉じ込める', text: '化粧水→美容液→クリームの順で重ね、うるおい感をキープ' },
      { icon: '🌙', title: 'ナイトケアの充実', text: '寝る前にリッチなクリームでしっかり保湿' },
    ],
    ngList: [
      '洗浄力の強いクレンジング・洗顔料',
      '熱いお湯での洗顔（ぬるま湯がベスト）',
  ...
  oily: {
    ...
    bgColor: '#F2EBDC',
    description:
      '皮脂の分泌が活発で、テカリや毛穴の開き・黒ずみが気になりやすい状態。意外なことに「うるおい不足」が皮脂が多く出る引き金になっていることも。落としすぎず、水分はしっかり与えるのがポイント。',
    features: [
      '午後になると顔全体がテカりやすい',
      '毛穴の開き・黒ずみが気になる',
      'ニキビや吹き出物ができやすい',
    ...
    cares: [
      { icon: '🫧', title: '丁寧なクレンジング', text: '毛穴の汚れをやさしく落としつつ、落としすぎない洗顔を' },
      { icon: '💦', title: 'みずみずしい保湿', text: 'さっぱり系の化粧水でうるおい補給。乳液は軽めに' },
      { icon: '🧊', title: 'キメを整えるケア', text: 'ビタミンC誘導体配合のアイテムで毛穴の目立ちにくいなめらかな印象に' },
    ],
    ngList: [
  ...
  combination: {
    ...
    bgColor: '#EFE8E8',
    description:
      'Tゾーン（おでこ・鼻）はテカるのに、頬や口周りは乾燥する…日本人に最も多い肌質。パーツごとに状態が違うので「全顔同じケア」は卒業。部位別ケアでうるおいバランスが整いやすくなります。',
    features: [
      'おでこ・鼻はテカるが頬は乾燥する',
      '季節や環境で肌の状態が変わりやすい',
      '部分的に毛穴が目立つ',
    ...
    cares: [
      ...
      { icon: '⚖️', title: 'みずみずしい保湿', text: 'ジェルタイプの保湿剤で、水分と油分のバランスを整える' },
      { icon: '🌿', title: '肌のキメを整える', text: 'ナイアシンアミド配合のアイテムで肌全体のうるおい・キメをサポート' },
    ],
    ngList: [
      '顔全体に同じケアをする',
      'Tゾーンの保湿を省略する',
  ...
  sensitive: {
    ...
    bgColor: '#F4EAE9',
    description:
      '通常なら気にならない刺激にも、ヒリヒリ・かゆみ・赤みなどを感じやすい状態。バリア機能の低下に加えて、神経の感じやすさ・血管反応など複数の要因が関わると言われています。「シンプル × 低刺激 × うるおいキープ」の3点が肌を労わる鍵。',
    features: [
      '化粧品でピリピリ・赤みが出ることがある',
      '季節の変わり目に肌が荒れやすい',
      '花粉や乾燥でかゆみを感じやすい',
    ...
    cares: [
      ...
      { icon: '🛡️', title: 'うるおいで肌を守る', text: 'セラミドなどでうるおいを与え、外的刺激から肌を守る' },
  ...
  aging: {
    ...
    bgColor: '#E9E4ED',
    description:
      '小ジワ・ハリ感の変化・くすんだ印象などが気になりやすい時期。紫外線・乾燥・生活習慣の影響を受けやすく、変化が出やすいタイミングでもあります。「うるおいケア」と「365日のUV対策」の両輪が、明日の肌の印象を左右します。',
    features: [
      '目元や口元の小ジワが気になる',
      'フェイスラインのもたつきが気になる',
      'ハリやツヤの印象が変わってきた',
    ...
    cares: [
      { icon: '✨', title: 'ハリ・透明感サポート', text: 'レチノール（医薬部外品では小ジワケア有効成分）やビタミンC誘導体配合のアイテムを取り入れる' },
      { icon: '💆', title: 'やさしいフェイスケア', text: 'こすらず、優しく押さえるようにケア' },
      { icon: '☀️', title: '365日のUV対策', text: '日焼け止めは曇りや室内・冬も毎日。紫外線はうるおい・ハリ印象に大きく影響' },
    ],
    ngList: [
      'UV対策を怠る（紫外線は印象変化の大きな要因）',
      '過度なダイエット（栄養不足は肌印象にも影響）',
      '肌を引っ張るマッサージ',
    ],
